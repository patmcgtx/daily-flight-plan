...
- **✅ Tab reorder**: Routine now appears before Log in the tab bar (Day / Routine / Log / Comm) — done manually in Xcode, not by the agent. **Note:** macOS `Cmd+2`/`Cmd+3` keyboard shortcuts in `DayView.swift` were not updated to match and still map to the pre-reorder positions (`Cmd+2` → Log, `Cmd+3` → Routine)
