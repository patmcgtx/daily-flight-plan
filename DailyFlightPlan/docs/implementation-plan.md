...
- **✅ Search**: `.searchable(text:)` on the list — this is the *main* search surface for the whole app. Matches title and notes, case-insensitive, deliberately searching the full `allItems` query rather than just the pre-cached `minLoadedDate...maxLoadedDate` window, so it reaches every date regardless of what's currently scrolled/loaded. Results replace the date-grouped list with a flat, date-sorted list (each row shows its date via `TimelineItemRow(showDate: true)`); empty state uses `ContentUnavailableView.search(text:)`; tapping a result opens the same edit sheet as a normal row. Respects the active Flagged/Missed/category filters.
