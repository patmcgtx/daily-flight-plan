...
- **CloudKit dedup for user-created templates**: `deduplicateItems` currently only catches seed-data duplicates (matched by deterministic `sourceID`). User-created templates that get duplicated by a CloudKit sync race would slip through. Add a second pass that matches on `title + daySection + recurringWeekdays` to catch these, merging instances onto the canonical copy before deleting the duplicate.
