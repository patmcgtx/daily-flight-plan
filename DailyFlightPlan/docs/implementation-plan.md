...
Paste any block of markdown text and let the app propose a list of items to add.

- **Entry point**: a "Paste & Import" button somewhere accessible — toolbar of the Day tab or a long-press on the `+` button
- **Import sheet**: a two-step modal
  1. **Paste step** — large `TextEditor` pre-populated from the clipboard; user can edit the raw text before parsing
  2. **Review step** — proposed item list generated by Foundation Models from the pasted markdown; each row shows title, day section, deadline, and recurring schedule fields; user can tap any row to edit it in place before committing
- **Parsing**: use Foundation Models (`LanguageModelSession` + a `@Generable` struct) to extract structured items from the markdown:
  - `title: String`
  - `notes: String?`
  - `daySection: String?` (e.g. "morning", "evening"; resolved to `DaySection` post-generation)
  - `deadlineTime: String?` (e.g. "9:30 AM"; resolved to `Date` anchored to the selected day)
  - `isRecurring: Bool`
  - `recurringWeekdays: [String]?` (e.g. ["monday", "wednesday"]; resolved to `[Locale.Weekday]`)
  - `isFlagged: Bool`
- **Review actions**: **Save** (creates all listed items), **Cancel** (dismisses without creating anything), and per-row **Delete** (removes that proposed item before committing)
- Items created on the currently selected date; recurring items create templates
- Fall back gracefully if Foundation Models is unavailable (OS < 26 or model not ready): skip parsing, show the raw text as a single-line title proposal, open `ItemForm` for manual entry
- **Import from Things**: parse a Things markdown export and let the model auto-assign sections — this is the primary use case driving this feature
- **Export** (stretch goal for this phase): share a day's items as a markdown checklist via the system share sheet; minimal formatting (one `- [ ] Title` line per pending item)
