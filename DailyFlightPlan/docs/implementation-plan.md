...
### Phase 2.2 — Timeline Refinements
...
- **✅ Dropped, then restored, the "Done" filter** — initially removed in favor of always showing completed/canceled items by default; once search shipped, using search surfaced real value in being able to hide completed/canceled items to cut down noise in results, so the toggle came back (`AppStorageKeys.showCompleted`, shared with the Day view's same filter), defaulting to off/hidden like it did originally
- **✅ Fix: "Done" + "Missed" together showed items, contradicting their apparent meaning**: "Missed" isolates (only pending+overdue pass), but "Done" only reveals (it never excludes pending items) — so with both on, "Done" contributed no restriction and results were just whatever "Missed" alone would show, not the empty set a user would expect from two seemingly opposite filters. Fixed at the UX level rather than the boolean logic: tapping either one now turns the other off, making Done/Missed mutually exclusive (Flagged still combines freely with either)
...
- **✅ Search**: `.searchable(text:)` on the list — this is the *main* search surface for the whole app. Matches title and notes, case-insensitive, deliberately reaching every date rather than just the pre-cached `minLoadedDate...maxLoadedDate` window. Results replace the date-grouped list with a flat, date-sorted list (each row shows its date via `TimelineItemRow(showDate: true)`); empty state uses `ContentUnavailableView.search(text:)`; tapping a result opens the same edit sheet as a normal row. Respects the active Flagged/Done/Missed/category filters.
- **✅ Fix: search field wasn't visible at all** — two stacked issues. First, `.searchable` was attached deep inside the `ScrollViewReader`/`List` chain, where it contended with the filter bar's own `.safeAreaInset(edge: .top)` for the same slot; moved it to the outer `NavigationStack` instead. Second, even after that, the field was still invisible on a physical device on iOS 26 (but fine in the iOS 27 simulator) — `.searchable`'s default `.automatic` placement apparently resolves differently between those OS builds (always-visible drawer vs. a minimized tap-to-expand button). Fixed by forcing an explicit `.navigationBarDrawer(displayMode: .always)` placement instead of leaving it to automatic resolution.
- **✅ Search now runs a real database fetch, not an in-memory filter**: `searchResults` used to call `.filter` on the `allItems` `@Query` array (already fully materialized in memory). Switched to a fresh `FetchDescriptor<PlanItem>` with a `#Predicate` using `localizedStandardContains` (case/diacritic-insensitive, pushed down to the store) and `sortBy: [SortDescriptor(\.date)]`, so search performance and memory use no longer scale with total item count as the database grows over years of use. Verified at runtime (not just compile-time) that the predicate translates correctly and both matching and non-matching queries behave as expected.
- **Remaining scaling concern**: `allItems` (the `@Query` backing the normal day-windowed browsing view, not search) still fetches every non-template `PlanItem` regardless of `minLoadedDate`/`maxLoadedDate` — the day-window filtering happens in Swift on the full in-memory result, not via a narrower predicate. Not addressed now since `@Query`'s predicate is fixed at initialization time; a real fix means restructuring to a dynamic predicate (e.g. a child view whose `init` takes the date range and constructs its own `@Query`, recreated when the window grows) or dropping `@Query` here in favor of manual `FetchDescriptor` calls like search now uses. Revisit in Phase release.3 (Performance & Stability Audit) if item counts become large enough to matter in practice.
- **Liquid Glass toolbar pass**: the current top area (nav title, filter capsules row, search field) was hand-rolled before Liquid Glass conventions solidified and reads as visually inconsistent with the rest of the app's Liquid Glass toolbars. Consider: moving Flagged/Done/Missed/category filters into a proper Liquid Glass `ToolbarItemGroup` (menu or filter icon, matching `FlightPlanView`'s filter menu pattern) instead of a custom `safeAreaInset` capsule row; and exploring whether the search field could live at the bottom of the screen instead of the top (closer to thumb reach, similar in spirit to the "Relocate '+' button to thumb zone" idea in Phase release.2) — no clean built-in SwiftUI way to do this with `.searchable` today, would likely need a custom search bar. Bigger redesign, deferred; revisit alongside Phase release.1 (Full-App UX Audit).
- Not yet done: editing on today/future items with swipe gestures (cancel, defer, flag, delete); per-day share icon
