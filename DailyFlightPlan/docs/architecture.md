...
- Tab order in the UI is Day / Routine / Log / Comm. **Note:** macOS keyboard shortcuts were not updated to match — `Cmd+1` Day, `Cmd+2` Log, `Cmd+3` Routine, `Cmd+4` Comm (shortcut order still reflects the pre-reorder tab positions)
